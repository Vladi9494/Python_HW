...
def driver():
    """
    Фикстура для инициализации, открытия сайта магазина
...
def test_shop_flow(driver):
    ...
    with allure.step("Заполнение формы (оформления заказа)'checkout'"
                     " личными данными: Имя, Фамилия, Почтовый индекс"):
        personal_data.input_info()
        personal_data.check_prise()

    checkout_page = CheckoutPage(driver)
    with allure.step("Чтение со страницы итоговой стоимости заказа"
                     " (total_price)"):
